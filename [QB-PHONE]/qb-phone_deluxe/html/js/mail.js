...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('http://qb-phone_deluxe/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('http://qb-phone_deluxe/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        $.post('http://qb-phone_deluxe/PostAdvert', JSON.stringify({
            message: Advert,
        }));
    } else {
